...
class NewHeaderInterceptor : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        runBlocking {
            val ticketCode = PreferencesDataStore(BaseApplication.instance()).getString(PreferencesKeys.ticketCode)
            val isSimulator = PreferencesDataStore(BaseApplication.instance()).getInt(PreferencesKeys.isSimulator)
            addHeader.addHeader("Content-Type", "application/json")
                .addHeader("requestTerminal", "0")
                .addHeader("timestamp", timeStamp)
                .addHeader("token", ticketCode)
                .addHeader("version", AppUtils.getAppVersionName())
                .addHeader("versionCode", AppUtils.getAppVersionCode().toString())
                .addHeader("isSimulator", isSimulator.toString())
